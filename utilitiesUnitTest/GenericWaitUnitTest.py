...
class GenericWaitUnitTest(unittest.TestCase):
    ...
    def test_passNoParamUntilTest(self):
        ...
        loop = [0]
        try:
            self.assertTrue(GenericWait().wait_until(loop[0] + 1 > 3), "Failed no parameter test")
        except Exception as e:
            self.fail("waitUntil no parameter test failed with exception" + ''.format(*e.args))
    ...
    def test_failObjectArrayUntilTest(self):
        objects = []
        try:
            self.assertFalse(GenericWait().wait_until(self.is_two_parameters(objects), objects),
    ...
    def test_customTimeoutWithoutParamTest(self):
        try:
            ...
            self.assertTrue(duration < max_time,
                            "The max wait time should be no more than " + str(max_time) + " but was " + str(duration))
        except Exception as e:
    ...
    def test_customTimeoutWithParamTest(self):
        try:
            max_time = int(self.test_timeout) + int(self.test_retry) + int(self.test_retry)
            start_time = datetime.now()
            GenericWait().wait(self.is_param_test, self.test_retry, self.test_timeout, "bad")
            duration = (datetime.now() - start_time).total_seconds()
            self.assertTrue(duration < max_time,
                            "The max wait time should be no more than " + str(max_time) + " but was " + str(duration))
    ...
    def is_two_parameters(parameters):
        return len(parameters) == 2 and isinstance(parameters[0], str) and isinstance(parameters[1], dict)
        # return isinstance(parameters, list) and isinstance(parameters[0], str) and isinstance(parameters[1], dict)
